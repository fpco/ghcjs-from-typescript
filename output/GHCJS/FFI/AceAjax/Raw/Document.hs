module GHCJS.FFI.AceAjax.Raw.Document where
import qualified GHCJS.Types as GHCJS
import GHCJS.FFI.TypeScript
import GHCJS.DOM.Types (HTMLElement)
import GHCJS.FFI.AceAjax.Raw.Types

foreign import javascript "$1.on($2,$3)" on :: Document -> (GHCJS.JSString) -> (GHCJS.JSRef ((GHCJS.JSRef obj0) -> IO (GHCJS.JSRef obj1))) -> IO (GHCJS.JSRef obj2)
foreign import javascript "$1.setValue($2)" setValue :: Document -> (GHCJS.JSString) -> IO (GHCJS.JSRef obj0)
foreign import javascript "$1.getValue()" getValue :: Document -> IO (GHCJS.JSString)
foreign import javascript "$1.createAnchor($2,$3)" createAnchor :: Document -> (GHCJS.JSNumber) -> (GHCJS.JSNumber) -> IO (GHCJS.JSRef obj0)
foreign import javascript "$1.getNewLineCharacter()" getNewLineCharacter :: Document -> IO (GHCJS.JSString)
foreign import javascript "$1.setNewLineMode($2)" setNewLineMode :: Document -> (GHCJS.JSString) -> IO (GHCJS.JSRef obj0)
...
foreign import javascript "$1.removeNewLine($2)" removeNewLine :: Document -> (GHCJS.JSNumber) -> IO (GHCJS.JSRef obj0)
foreign import javascript "$1.replace($2,$3)" replace :: Document -> (Range) -> (GHCJS.JSString) -> IO (GHCJS.JSRef obj0)
foreign import javascript "$1.applyDeltas($2)" applyDeltas :: Document -> (GHCJS.JSArray (Delta)) -> IO (GHCJS.JSRef obj0)
foreign import javascript "$1.revertDeltas($2)" revertDeltas :: Document -> (GHCJS.JSArray (Delta)) -> IO (GHCJS.JSRef obj0)
