module JavaScript.AceAjax.Raw.Range where
...
foreign import javascript "$1.startRow" startRow :: Range -> IO (GHCJS.JSNumber)
foreign import javascript "$1.startColumn" startColumn :: Range -> IO (GHCJS.JSNumber)
foreign import javascript "$1.endRow" endRow :: Range -> IO (GHCJS.JSNumber)
foreign import javascript "$1.endColumn" endColumn :: Range -> IO (GHCJS.JSNumber)
foreign import javascript "$1.start" start :: Range -> IO (Position)
foreign import javascript "$1.end" end :: Range -> IO (Position)
foreign import javascript "$1.isEmpty()" isEmpty :: (Range) -> IO (GHCJS.JSBool)
foreign import javascript "$1.isEqual($2)" isEqual :: (Range) -> (Range) -> IO (())
foreign import javascript "$1.toString()" toString :: (Range) -> IO (())
...
foreign import javascript "$1.setStart($2,$3)" setStart :: (Range) -> (GHCJS.JSNumber) -> (GHCJS.JSNumber) -> IO (())
foreign import javascript "$1.setEnd($2,$3)" setEnd :: (Range) -> (GHCJS.JSNumber) -> (GHCJS.JSNumber) -> IO (())
